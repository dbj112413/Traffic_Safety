...
try:
    train_df = pd.read_csv(BASE_DIR / 'Training_merged.csv', low_memory=False)
    val_df = pd.read_csv(BASE_DIR / 'Validation_merged.csv', low_memory=False)
    test_df = pd.read_csv(BASE_DIR / 'Testing_merged.csv', low_memory=False)
...
for col in numeric_features:
    ...
    X_val[col] = pd.to_numeric(X_val[col], errors='coerce').astype('float64')
    X_test[col] = pd.to_numeric(X_test[col], errors='coerce').astype('float64')

print("\nStep 3. AI 大腦正式訓練 (LightGBM 模式)")
if not X_train.empty:
    ...
    # LGBM requires categorical_feature to be specified or pandas category type
    model.fit(X_train, y_train, eval_set=[(X_val, y_val)], categorical_feature=categorical_features)
    
    joblib.dump(model, 'best_traffic_model_gbm.pkl')
    print("\n💾 訓練完成！超強大腦已成功存檔為 'best_traffic_model_gbm.pkl'！")
...
# =========================================================================
# SHAP 視覺化程式碼 (針對 LightGBM)
# =========================================================================
def run():
    print("\n⏳ [SHAP 視覺化啟動] 正在計算沙普利值...")
    
    # 取樣 2000 筆資料進行解釋，避免計算過久
    ...
    print("🎨 正在繪製：特徵平均影響力排行榜 (LightGBM)...")
    plt.figure(figsize=(10, 6))
    shap.plots.bar(shap_values_pos, max_display=13, show=False)
    plt.title("🚨 車禍重傷風險 - 核心特徵排行榜 (LightGBM)", fontsize=14, pad=20)
    plt.tight_layout()
    plt.savefig('shap_importance_bar_gbm.jpg', dpi=300, bbox_inches='tight')
    plt.close()

    # --- 繪製：SHAP 全局非線性分佈蜂群圖 (Beeswarm Plot) ---
    print("🎨 正在繪製：SHAP 全局分佈蜂群圖 (LightGBM)...")
    plt.figure(figsize=(11, 7))
    shap.summary_plot(shap_values_pos.values, X_shap_sample, max_display=13, show=False)
    plt.title("🔮 車禍惡化風險 - SHAP 全局分佈圖 (LightGBM)", fontsize=14, pad=20)
    plt.tight_layout()
    plt.savefig('shap_summary_beeswarm_gbm.jpg', dpi=300, bbox_inches='tight')
    plt.close()

    print("\n🎉 【SHAP 視覺化圖表輸出成功！】")
    print("👉 已自動生成：")
    print("   1. shap_importance_bar_gbm.jpg")
    print("   2. shap_summary_beeswarm_gbm.jpg")
